...
| Execution envelopes, attempts, cancellation controls, runtime jobs, and lease tables | **Internal implementation detail** | Envelope schema v4 and job/recovery schema v5 are active behind the stable task surfaces. Physical table layout remains internal; execution admission, audit correlation, idempotency, restart classification, and uncertain-outcome behavior remain compatibility obligations. |
