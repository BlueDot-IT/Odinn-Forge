...
| Proof, Sentinel, Rewind, and Darwin | **Core advanced service; experimental interface** | Available without feature flags and integrated with the runtime, while their advanced CLI/gateway contracts remain outside the stable public-SDK promise. |
| Capsules, Capability Tokens, and Counterfactual | **Optional plugin module; experimental interface** | Disabled by default and loaded through the internal runtime plugin boundary. Their surfaces and state remain outside normal v1 compatibility guarantees. |
