...
| Unified operator snapshot and governed actions (`odinn operator`, `odinn inspect`, TUI, `/operator/snapshot`, `/operator/actions`, and the console Operator page) | **Stable v1 interface** | Snapshot schema v1, bounded pagination, redaction, authentication/origin checks, and action confirmation are shared across surfaces. Human-readable TUI and HTML layout may evolve; uncertain outcomes remain visible for review. |
...
| Executable agent manifests and bounded child-agent graphs | **Experimental interface** | Explicitly disabled by default and jobs-only. The active profile is limited to read-only child nodes with bounded fan-out/concurrency; manifests are integrity-checked, child authority is intersected with the parent grants, and durable projections retain digests rather than prompts or provider output. Operator list/detail/cancel/reassign/checkpoint routes preserve immutable history, emit signed pre-effect intent and outcome evidence, require authority subsets and exact digests, fence reassignment to one successor under the original trusted tenant/principal, and route any publication through the existing governed mutation boundary. |
| Host-capability plugin seams (`computer.screen` and provider-injected email reads) | **Experimental interface** | Availability requires an injected provider or paired node and explicit host policy. No ambient desktop session, desktop mutation path, or concrete email provider is bundled; all materialized tools remain inside the shared admission, approval, audit, and recovery boundary. |
...
| Local Git inspection (`git.status`, `git.diff`, `git.log`) | **Experimental interface** | Explicit `git.read` authority, local refs only, no network or credential helpers, bounded output, `.git`/runtime-state exclusion, and content-free durable evidence. Mutation, fetch, push, and remote metadata remain unsupported. |
