...
Darwin is a core advanced service. It records measured outcomes in SQLite and
uses a transparent weighted score across verification, provisional success,
reliability, rollback history, policy compliance, speed, and known cost. An
uncertainty penalty prevents a single lucky run from becoming gospel.
...
When `model.chat` does not pin a model, Darwin selects only among models in the
active configuration. With no applicable observations it falls back to the
configured default. Provider success is recorded as provisional; a subsequent
Proof result for the run marks the observation verified or failed. Prompts and
secrets are not stored in observation rows.
...
`model-observation` event linked to its SQLite observation row. Proof then
