...
const CREDENTIAL_AUTHORITY_SUBJECT_SET = new Set(CREDENTIAL_AUTHORITY_SUBJECTS);
const CREDENTIAL_AUTHORITY_MATERIAL_SET = new Set(CREDENTIAL_AUTHORITY_MATERIALS);
...
function normalizedAlphanumeric(value) {
    return value.replace(/[^A-Za-z0-9]+/gu, "").toLowerCase();
}
function isCredentialAuthorityComposition(value) {
    const visit = (offset, depth, materialSeen) => {
        if (offset === value.length)
            return depth >= 2 && materialSeen;
        if (depth >= 3)
            return false;
        const candidates = depth === 0 ? CREDENTIAL_AUTHORITY_SUBJECTS : CREDENTIAL_AUTHORITY_PARTS;
        for (const part of candidates) {
            if (value.startsWith(part, offset)
                && visit(offset + part.length, depth + 1, materialSeen || (depth > 0
...
function hasProtectedIdentity(value) {
...
function bounded(value, label, max) {
    if (typeof value !== "string" || !value.length || value.length > max || /[\0-\x1f\x7f]/u.test(value))
        fail(`${label} is not bounded`);
    return value;
}
function strings(value, label, max, valid) {
    if (!Array.isArray(value) || value.length > max || value.some((item) => typeof item !== "string" || !valid(item)) || new Set(value).size !== value.length)
        fail(`${label} must be a bounded unique array`);
    return value;
}
export function validatePluginId(value) {
    const id = bounded(value, "id", 64);
    if (id.length < 2 || !ID.test(id) || PROTECTED.has(id) || hasProtectedIdentity(id))
        fail("id must be lowercase kebab-case");
    return id;
}
export function validatePluginVersion(value) {
    const version = bounded(value, "version", 64);
    if (!VERSION.test(version))
        fail("version must be semantic without build metadata");
    return version;
}
export function validatePluginImage(value) {
    const image = bounded(value, "containerImage", 512);
    if (!/^(?:[a-z0-9](?:[a-z0-9.-]*[a-z0-9])?(?::[0-9]{1,5})?\/)?[a-z0-9]+(?:(?:[._]|__|[-]+)[a-z0-9]+)*(?:\/[a-z0-9]+(?:(?:[._]|__|[-]+)[a-z0-9]+)*)*(?::[A-Za-z0-9_][A-Za-z0-9_.-]{0,127})?@sha256:[a-f0-9]{64}$/u.test(image))
        fail("containerImage must be an exact digest-pinned OCI reference");
    const slash = image.indexOf("/");
    const registry = slash < 0 ? "" : image.slice(0, slash);
    if (registry.includes(":")) {
        const port = Number(registry.split(":").at(-1));
        if (!Number.isInteger(port) || port < 1 || port > 65535)
            fail("invalid image registry port");
    }
    return image;
}
/** Forge's bounded MCP schema subset, deliberately not general JSON Schema. */
export function validatePluginInputSchema(input) {
    let nodes = 0;
    const visit = (raw, depth) => {
        if (++nodes > 1024 || depth > 12)
            fail("inputSchema exceeds complexity limits");
        ...
        if (value.type === "object") {
            ...
            if (value.additionalProperties !== false)
                fail("object schema additionalProperties must be false");
            const properties = plain(value.properties, "schema properties");
            if (Object.keys(properties).length > 128)
                fail("schema has too many properties");
            const normalized = {};
            for (const key of Object.keys(properties).sort()) {
                if (!/^[A-Za-z][A-Za-z0-9_]{0,63}$/u.test(key) || PROTECTED.has(key) || hasProtectedIdentity(key))
                    fail("unsafe schema property");
            ...
            for (const key of fields) {
                ...
                if (bound === undefined)
                    continue;
                if (typeof bound !== "number" || !Number.isFinite(bound) || Math.abs(bound) > Number.MAX_SAFE_INTEGER)
                    fail("schema bound must be finite");
                if (value.type !== "number" && !Number.isSafeInteger(bound))
                    fail("schema bound must be integral");
                if ((value.type === "array" || value.type === "string") && (bound < 0 || bound > (value.type === "array" ? 1024 : 16384)))
                    fail("schema bound exceeds host limits");
                result[key] = bound;
            }
            if (typeof result[fields[0]] === "number" && typeof result[fields[1]] === "number" && Number(result[fields[0]]) > Number(result[fields[1]]))
                fail("schema minimum exceeds maximum");
        }
        else if (value.type === "boolean")
            exact(value, ["type"], "boolean schema");
        else
            fail("inputSchema type is not in the supported MCP subset");
        return Object.freeze(result);
    };
    const result = visit(input, 0);
    if (result.type !== "object" || JSON.stringify(result).length > 65536)
        fail("tool inputSchema must be a bounded object schema");
    return result;
}
export function validatePluginManifest(input) {
    ...
    if (source.schemaVersion !== 1 || source.sdkVersion !== PLUGIN_SDK_VERSION)
        fail("unsupported schema or sdk version");
    const id = validatePluginId(source.id), version = validatePluginVersion(source.version);
    if (source.kind !== "connector" || source.runtime !== "oci-mcp-stdio" || source.transport !== "mcp-jsonrpc-stdio")
        fail("v1 only supports connector + oci-mcp-stdio + mcp-jsonrpc-stdio");
    const containerImage = validatePluginImage(source.containerImage);
    if (!Array.isArray(source.tools) || !source.tools.length || source.tools.length > 128)
        fail("tools must be a nonempty bounded array");
    const names = new Set();
    const tools = source.tools.map((raw) => {
        const tool = plain(raw, "tool");
        exact(tool, ["name", "description", "capabilities", "inputSchema", "effects", "requiresApproval", "retrySafe"], "tool");
        const name = bounded(tool.name, "tool name", 128);
        if (!name.startsWith(`${id}.`) || !/^[a-z][a-z0-9-]*\.[a-z][a-z0-9]*(?:[._-][a-z0-9]+)*$/u.test(name) || !/^[a-z][a-z0-9]*(?:[._-][a-z0-9]+){0,7}$/u.test(name) || hasProtectedIdentity(name) || names.has(name))
            fail("tool names must be unique and prefixed with plugin id");
        names.add(name);
        const capabilities = strings(tool.capabilities, "capabilities", 4, (item) => CAPABILITIES.has(item));
        if (!capabilities.length)
            fail("tool capabilities are required");
        const effects = strings(tool.effects, "effects", 3, (item) => ["read", "network", "credential"].includes(item));
        if (!effects.includes("read") || capabilities.includes("network.access") !== effects.includes("network") || capabilities.includes("secret.reference.use") !== effects.includes("credential"))
            fail("tool effects and capabilities must agree");
        if (typeof tool.requiresApproval !== "boolean" || tool.retrySafe !== true)
            fail("v1 read tools require safety flags and retrySafe:true");
    ...
    const egress = plain(source.egress, "egress");
    exact(egress, ["protocol", "hosts", "port"], "egress");
    if (egress.protocol !== "https" || egress.port !== 443)
        fail("egress must use HTTPS on port 443");
    ...
    if (!Array.isArray(source.secrets) || source.secrets.length > 32)
        fail("secrets must be bounded");
    const secretNames = new Set();
    const secrets = source.secrets.map((raw) => {
        const ref = plain(raw, "secret");
        exact(ref, ["name", "purpose"], "secret");
        const name = bounded(ref.name, "secret name", 96);
        if (!/^[A-Z][A-Z0-9_]{1,95}$/u.test(name) || secretNames.has(name))
            fail("secret names must be unique uppercase credential names");
        secretNames.add(name);
        return Object.freeze({ name, purpose: bounded(ref.purpose, "secret purpose", 256) });
    });
    if (!Array.isArray(source.services) || source.services.length > 32)
        fail("services must be bounded");
    const ids = new Set(), serviceHosts = new Set(), usedSecrets = new Set();
    const services = source.services.map((raw) => {
        const service = plain(raw, "service");
        exact(service, ["id", "origin", "paths", "queryKeys", "credential"], "service");
        const serviceId = validatePluginId(service.id);
        ...
        try {
            ...
            return fail("invalid service origin");
        }
        if (url.protocol !== "https:" || url.username || url.password || url.port || url.pathname !== "/" || url.search || url.hash || origin !== url.origin || !hosts.includes(url.hostname))
            fail("service origin must be an exact declared HTTPS origin without credentials, path, or non443 port");
        ...
        if (!paths.length)
            fail("service requires exact paths");
        ...
        if (credential !== undefined && (typeof credential !== "string" || !secretNames.has(credential)))
            fail("service credential must refer to a declared secret");
        if (typeof credential === "string")
            usedSecrets.add(credential);
    ...
    if (hosts.some((host) => !serviceHosts.has(host)) || secrets.some((ref) => !usedSecrets.has(ref.name)))
        fail("egress hosts and secrets must be used by services");
    if (Boolean(services.length) !== tools.some((tool) => tool.capabilities.includes("network.access")))
        fail("services and network.access capabilities must agree");
    if (Boolean(secrets.length) !== tools.some((tool) => tool.capabilities.includes("secret.reference.use")))
        fail("secrets and secret.reference.use capabilities must agree");
    const entrypoint = bounded(source.entrypoint, "entrypoint", 256);
    if (!/^[A-Za-z0-9][A-Za-z0-9._/-]*$/u.test(entrypoint) || entrypoint.split("/").some((part) => !part || part === "." || part === ".." || part.endsWith(".") || PROTECTED.has(part)) || !/\.(?:mjs|cjs|js)$/u.test(entrypoint))
        fail("entrypoint must be a relative JavaScript package file");
...
export function createManifest(input) { return validatePluginManifest({ ...input, schemaVersion: 1, sdkVersion: PLUGIN_SDK_VERSION }); }
export function createConnectorScaffold(id, containerImage = DEFAULT_PLUGIN_IMAGE) {
