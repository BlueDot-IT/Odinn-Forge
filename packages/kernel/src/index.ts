...
import { executeWorkspaceProcess, writeWorkspaceText } from "./workspace-tools.ts";
...
export function createBuiltInRegistry({ workspaceRoot = process.cwd(), stateDir = ".odinn", config = {}, approvalStore = createApprovalStore(), auditStore, resolveNetworkAddresses = dnsLookupAll, discordFetch = globalThis.fetch }: any = {}): BuiltInRegistry {
  ...
  const registry = new Map([
    ...
    ["workspace.writeText", {
      ...
      description: "Atomically create or replace a bounded UTF-8 text file confined to the workspace root.",
      inputSchema: { type: "object", properties: { path: { type: "string" }, content: { type: "string" }, createDirectories: { type: "boolean" }, maxBytes: { type: "integer", minimum: 1, maximum: 1_000_000 } }, required: ["path", "content"], additionalProperties: false },
