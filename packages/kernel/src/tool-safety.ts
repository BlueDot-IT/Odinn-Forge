...
const READ_TOOLS = new Set(["job.healthcheck", "text.echo", "workspace.readText", "workspace.list", "workspace.stat", "workspace.search", "workspace.read", "workspace.diff", "web.search", "web.fetch", "browser.tabs", "browser.open", "browser.snapshot", "browser.recovery.status", "email.accounts", "email.search", "email.read", "email.thread", "memory.search", "memory.recall", "memory.browse", "memory.open", "memory.curate", "session.list", "session.read", "project.list", "goal.list", "improve.list", "mcp.discover", "skill.catalog", "skill.hydrate"]);
...
export function toolSafetyDescriptor(toolName: unknown, tool: unknown): ToolSafetyDescriptor {
  ...
  if (READ_TOOLS.has(name)) return { toolName: name, effects: name.startsWith("web.") || name.startsWith("browser.") || name === "mcp.discover" ? ["read", "network"] : ["read"], reversibility: "pure", requiresCapability: true, requiresApproval: false, retrySafe: RETRY_SAFE_TOOLS.has(name) };
