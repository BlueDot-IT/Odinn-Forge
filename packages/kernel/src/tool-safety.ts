const READ_TOOLS = new Set(["job.healthcheck", "text.echo", "workspace.readText", "web.search", "web.fetch", "browser.tabs", "browser.open", "browser.snapshot", "discord.listChannels", "discord.readMessages", "memory.search", "memory.recall", "memory.browse", "memory.open", "memory.curate", "session.list", "session.read", "project.list", "goal.list", "improve.list"]);
const WRITE_TOOLS = new Set(["workspace.writeText", "memory.remember", "memory.suggest", "memory.decide", "memory.compact", "memory.correct", "memory.forget", "session.create", "session.message", "session.rename", "session.assign", "session.update", "session.delete", "project.create", "project.update", "goal.create", "goal.update", "improve.propose", "improve.learn", "improve.decide", "improve.rollback"]);
...
export function toolSafetyDescriptor(toolName: unknown, tool: unknown): ToolSafetyDescriptor {
  ...
  if (["browser.click", "browser.type", "browser.press", "discord.createThread"].includes(name)) return { toolName: name, effects: ["network", "credential", "external-state"], reversibility: "irreversible", requiresCapability: true, requiresApproval: true, retrySafe: false };
  if (["discord.sendMessage", "discord.addReaction"].includes(name)) return { toolName: name, effects: ["network", "credential", "external-state"], reversibility: "compensatable", requiresCapability: true, requiresApproval: false, retrySafe: false };
