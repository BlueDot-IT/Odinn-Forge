...
export function toolSafetyDescriptor(toolName: unknown, tool: unknown): ToolSafetyDescriptor {
  ...
  if (["browser.click", "browser.type", "browser.press", "discord.sendMessage", "discord.addReaction", "discord.createThread"].includes(name)) return { toolName: name, effects: ["network", "credential", "external-state"], reversibility: "irreversible", requiresCapability: true, requiresApproval: true, retrySafe: false };
