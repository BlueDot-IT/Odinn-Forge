...
export function toolSafetyDescriptor(toolName: unknown, tool: unknown): ToolSafetyDescriptor {
  ...
  if (READ_TOOLS.has(name)) return { toolName: name, effects: name.startsWith("web.") || name.startsWith("browser.") || name.startsWith("discord.") || name === "mcp.discover" ? ["read", "network"] : ["read"], reversibility: "pure", requiresCapability: true, requiresApproval: false, retrySafe: RETRY_SAFE_TOOLS.has(name) };
