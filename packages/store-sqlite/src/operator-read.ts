...
type OperatorReadPage<T> = { readonly items: readonly T[]; readonly total: number; readonly attention: number };
...
export interface SqliteOperatorAuditReadRecord {
  ...
  readonly integrity: { readonly valid: boolean; readonly checked: boolean; readonly unsigned: number; readonly failures: readonly [] };
}
...
export class SqliteOperatorReadStore {
  ...
  queryJobs(input: Query = {}): OperatorReadPage<SqliteOperatorJobReadRecord> {
    ...
    const totals = this.runtime!.prepare(`SELECT count(*) AS total,
    ...
    const rows = this.runtime!.prepare(`SELECT id,status,payload_json,attempts,retry_safe,created_at,updated_at,completed_at,
  ...
  queryWorkflows(input: Query = {}): OperatorReadPage<SqliteOperatorWorkflowReadRecord> {
    ...
    if (query) {
      conditions.push("instr(lower('workflow durable workflow run ' || run_id || ' ' || definition_id || ' ' || definition_digest || ' ' || status),lower(?))>0");
      parameters.push(query);
    }
    const where = conditions.join(" AND ");
    const totals = this.runtime!.prepare(`SELECT count(*) AS total,
    ...
    const rows = this.runtime!.prepare(`SELECT run_id,definition_digest,updated_at,${effectiveStatus} AS effective_status
  ...
  queryEventWatches(input: Query = {}): OperatorReadPage<SqliteOperatorEventWatchReadRecord> {
    ...
    const where = conditions.join(" AND ");
    const totals = this.runtime!.prepare(`SELECT count(*) AS total FROM event_watches WHERE ${where}`).get(...parameters) as Row;
    const rows = this.runtime!.prepare(`SELECT watch_id,enabled,updated_at FROM event_watches WHERE ${where}
  ...
  queryRuns(input: Query = {}): OperatorReadPage<SqliteOperatorRunReadRecord> {
    ...
    const where = conditions.join(" AND ");
    const totals = this.audit!.prepare(`SELECT count(*) AS total,
    ...
    const rows = this.audit!.prepare(`SELECT run_id,summary_json FROM audit_runs WHERE ${where}
  ...
  readLatestExecutionAttempts(runIds: readonly string[]): readonly SqliteOperatorAttemptReadRecord[] {
    ...
    return (this.runtime!.prepare(`SELECT id,run_id,attempt_number,state,created_at,started_at,settled_at,outcome_digest,error_code
  ...
  readAudit(): SqliteOperatorAuditReadRecord {
    if (!hasTable(this.audit, "audit_events") || !hasTable(this.audit, "audit_runs") || !hasTable(this.audit, "audit_state")) {
      return {
        ...
        integrity: { valid: true, checked: false, unsigned: 0, failures: [] }
      };
    }
    const events = Number((this.audit!.prepare("SELECT count(*) AS count FROM audit_events").get() as Row).count || 0);
    const runs = Number((this.audit!.prepare("SELECT count(*) AS count FROM audit_runs").get() as Row).count || 0);
    const attentionRuns = Number((this.audit!.prepare("SELECT count(*) AS count FROM audit_runs WHERE json_extract(summary_json,'$.status') IN ('failed','blocked','needs-review')").get() as Row).count || 0);
    return {
