...
export const SQLITE_AUDIT_SCHEMA_VERSION = 1;
...
function legacySegmentLedgerEligible(db: DatabaseSync, keyringPath: string) {
  ...
  return segments.length === 1 && open === 1;
}
...
export class SqliteAuditStore {
  ...
  constructor(databasePath: string, { keyringPath = `${databasePath}.keys.json` }: { keyringPath?: string } = {}) {
    ...
    if (!databaseExisted) { this.signSegmentInventory(true); this.db.exec(`PRAGMA user_version=${SQLITE_AUDIT_SCHEMA_VERSION}`); }
    else if (schemaVersion < SQLITE_AUDIT_SCHEMA_VERSION) { if (!legacySegmentLedgerEligible(this.db, this.keyringPath)) { this.db.close(); throw new Error("existing audit segment topology is not eligible for authenticated-ledger migration"); } this.db.exec("BEGIN IMMEDIATE"); try { this.signSegmentInventory(true); this.db.exec(`PRAGMA user_version=${SQLITE_AUDIT_SCHEMA_VERSION}; COMMIT`); } catch (error) { this.db.exec("ROLLBACK"); this.db.close(); throw error; } }
    else if (schemaVersion > SQLITE_AUDIT_SCHEMA_VERSION) { this.db.close(); throw new Error(`audit SQLite schema ${schemaVersion} is newer than supported schema ${SQLITE_AUDIT_SCHEMA_VERSION}`); }
  ...
  async append(value: unknown): Promise<AuditEvent> {
    ...
    this.cachedIntegrity = undefined;
  ...
  async readRunPage({ offset = 0, limit = 100 }: { offset?: number; limit?: number } = {}) {
  ...
  async readSummary() {
    ...
    const attentionRuns = Number((this.db.prepare("SELECT count(*) AS count FROM audit_runs WHERE json_extract(summary_json, '$.status') IN ('failed','blocked','needs-review')").get() as Row).count);
    return { events, runs, attentionRuns };
  }
  async readFailurePage({ limit = 100 }: { limit?: number } = {}) {
    const rows = this.db.prepare("SELECT event_json FROM audit_events WHERE type IN ('task.failed','task.blocked','task.policy') ORDER BY sequence DESC LIMIT ?").all(Math.min(MAX_PAGE, bounded(limit, 100) * 4)) as Row[];
...
export function inspectExistingSqliteAuditSchema(databasePath: string): number {
  if (!existsSync(databasePath)) throw new Error(`audit SQLite database does not exist: ${databasePath}`);
  const db = new DatabaseSync(resolve(databasePath));
  try { return Number((db.prepare("PRAGMA user_version").get() as Row).user_version); }
  finally { db.close(); }
}
...
export function migrateLegacyAuditToSqlite({ legacyPath, databasePath, keyringPath = `${legacyPath}.keys.json` }: { legacyPath: string; databasePath: string; keyringPath?: string }) {
  ...
  const importEvent = (event: AuditEvent) => { const integrity = event.data?.__odinnIntegrity as Integrity | undefined; if (event.type === "audit.segment.rotated" && !integrity) throw new Error("legacy audit rotation events must be signed"); validateIntegrity(event); const result = store.db.prepare("INSERT INTO audit_events(run_id,actor,type,at,key_id,previous_signature,signature,event_json) VALUES(?,?,?,?,?,?,?,?)").run(event.runId,event.actor,event.type,event.at,integrity?.keyId ?? null,integrity?.previous ?? null,integrity?.signature ?? null,JSON.stringify(event)); events = Number(result.lastInsertRowid); if (event.type === "audit.segment.rotated") { const rotationIntegrity = integrity!; const rotation = event.data?.segmentRotation as Row; const closed = rotation?.closed as Row; const opened = rotation?.opened as Row; const current = store.db.prepare("SELECT * FROM audit_segments WHERE closed_at IS NULL").get() as Row | undefined; const currentInventory = current ? segmentInventory([current])[0]! : undefined; if (!current || !closed || !opened || Number(closed.id) !== Number(current.id) || Number(closed.firstSequence) !== currentInventory?.firstSequence || (closed.anchorSignature ?? null) !== currentInventory?.anchorSignature || String(closed.openedAt) !== currentInventory?.openedAt || Number(closed.lastSequence) !== events - 1 || (closed.finalSignature ?? null) !== (rotationIntegrity.previous ?? null) || Number(opened.id) !== Number(closed.id) + 1 || Number(opened.firstSequence) !== events || (opened.anchorSignature ?? null) !== (rotationIntegrity.previous ?? null) || String(opened.openedAt) !== String(closed.closedAt)) throw new Error("legacy audit rotation topology verification failed"); store.db.prepare("UPDATE audit_segments SET last_sequence=?,final_signature=?,closed_at=? WHERE id=?").run(closed.lastSequence,closed.finalSignature,closed.closedAt,closed.id); store.db.prepare("INSERT INTO audit_segments(id,first_sequence,anchor_signature,opened_at) VALUES(?,?,?,?)").run(opened.id,opened.firstSequence,opened.anchorSignature,opened.openedAt); return; } const prior = store.db.prepare("SELECT summary_json FROM audit_runs WHERE run_id=?").get(event.runId) as Row | undefined; const summary = statusFor(event, prior); store.db.prepare("INSERT INTO audit_runs VALUES(?,?,?) ON CONFLICT(run_id) DO UPDATE SET summary_json=excluded.summary_json,last_event_at=excluded.last_event_at").run(event.runId,JSON.stringify(summary),event.at); };
  store.db.exec("BEGIN IMMEDIATE");
  try {
    for (;;) { const bytes = readSync(descriptor, buffer, 0, buffer.length, position); if (!bytes) break; position += bytes; const parts = (carry + decoder.write(buffer.subarray(0, bytes))).split("\n"); carry = parts.pop()!; for (const line of parts) if (line.trim()) importEvent(normalizeAuditEvent(JSON.parse(line))); }
    carry += decoder.end();
    if (carry.trim()) importEvent(normalizeAuditEvent(JSON.parse(carry)));
    ...
    const segments = store.db.prepare("SELECT * FROM audit_segments ORDER BY id").all() as Row[]; const segmentKeyId = migrationKeyring.current; const segmentSignature = createHmac("sha256", Buffer.from(migrationKeyring.keys[segmentKeyId]!, "base64")).update(JSON.stringify(segmentInventory(segments))).digest("base64url"); store.db.prepare("UPDATE audit_segment_integrity SET key_id=?,signature=? WHERE singleton=1").run(segmentKeyId,segmentSignature);
