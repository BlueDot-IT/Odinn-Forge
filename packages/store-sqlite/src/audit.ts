...
export class SqliteAuditStore {
  ...
  exportArchive(path: string, throughSequence?: number) { const head = this.db.prepare("SELECT head_sequence,retained_sequence FROM audit_state WHERE singleton=1").get() as Row; const retained = Number(head.retained_sequence); const through = Math.min(Number(throughSequence ?? head.head_sequence), Number(head.head_sequence)); if (through <= retained) throw new Error("archive boundary must advance retained audit history"); const prior = retained ? this.verifiedArchive(retained) : undefined; if (retained && !prior) throw new Error("prior retained audit archive is unavailable"); ensureParent(path); const output = openSync(path, "wx", 0o600); const hash = createHash("sha256"); try { if (prior) { const input = openSync(String(prior.path), "r"); const buffer = Buffer.alloc(64 * 1024); try { for (;;) { const bytes = readSync(input, buffer, 0, buffer.length, null); if (!bytes) break; const chunk = buffer.subarray(0, bytes); writeFully(output, chunk); hash.update(chunk); } } finally { closeSync(input); } } let cursor = retained; for (;;) { const rows = this.db.prepare("SELECT sequence,event_json FROM audit_events WHERE sequence>? AND sequence<=? ORDER BY sequence LIMIT 2000").all(cursor, through) as Row[]; if (!rows.length) break; for (const row of rows) { const line = Buffer.from(`${JSON.stringify({ sequence: Number(row.sequence), event: JSON.parse(String(row.event_json)) })}\n`); writeFully(output, line); hash.update(line); cursor = Number(row.sequence); } } if (cursor !== through) throw new Error("archive range is incomplete"); fsyncSync(output); } catch (error) { closeSync(output); rmSync(path, { force: true }); throw error; } closeSync(output); const sha256 = hash.digest("hex"); const now = new Date().toISOString(); const keyring = keyringFor(this.keyringPath); const manifest = { schemaVersion: 1, path: resolve(path), firstSequence: 1, throughSequence: through, events: through, sha256, createdAt: now, keyId: keyring.current }; const signature = createHmac("sha256", Buffer.from(keyring.keys[keyring.current]!, "base64")).update(JSON.stringify(manifest)).digest("base64url"); writeFileSync(`${path}.manifest.json`, `${JSON.stringify({ ...manifest, signature }, null, 2)}\n`, { mode: 0o600, flag: "wx" }); this.db.prepare("INSERT INTO audit_archives(through_sequence,path,sha256,created_at,verified_at) VALUES(?,?,?,?,?)").run(through, resolve(path), sha256, now, now); return { ...manifest, signature }; }
