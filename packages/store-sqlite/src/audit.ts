...
export class SqliteAuditStore {
  ...
  constructor(databasePath: string, { keyringPath = `${databasePath}.keys.json` }: { keyringPath?: string } = {}) {
    ...
    if (!databaseExisted) this.signSegmentInventory(true);
  ...
  async verifyIntegrity({ allowUnsigned = true } = {}) {
    ...
    const segments = this.db.prepare("SELECT * FROM audit_segments ORDER BY id").all() as Row[]; if (!this.segmentInventoryValid(segments)) failures.push({ runId: "", reason: "audit segment ledger integrity mismatch" }); const retainedSegments = archiveSegments(segments, retainedSequence); const retainedBoundaries = new Set(retainedSegments.map((segment) => segment.lastSequence)); const archived = retainedArchive ? verifyArchivedHistory(String(retainedArchive.path), retainedSequence, keyring, retainedBoundaries) : { valid: retainedSequence === 0, lastSignature: null, boundaries: new Map<number, string>() }; if (!archived.valid || (retainedSequence > 0 && archived.lastSignature !== retained.retained_signature)) failures.push({ runId: "", reason: "retained audit archive chain mismatch" }); if (retainedArchive && JSON.stringify(retainedArchive.manifest.segments) !== JSON.stringify(retainedSegments)) failures.push({ runId: "", reason: "retained audit segment inventory mismatch" }); let expectedFirst = 1; let priorFinal: string | null = null; let openSegments = 0;
  ...
  rotateSegment() { this.db.exec("BEGIN IMMEDIATE"); try { if (!this.segmentInventoryValid()) throw new Error("audit segment ledger integrity verification required before rotation"); const head = this.db.prepare("SELECT head_sequence,head_signature FROM audit_state WHERE singleton=1").get() as Row; this.db.prepare("UPDATE audit_segments SET last_sequence=?,final_signature=?,closed_at=? WHERE closed_at IS NULL").run(head.head_sequence, head.head_signature, new Date().toISOString()); this.db.prepare("INSERT INTO audit_segments(first_sequence,anchor_signature,opened_at) VALUES(?,?,?)").run(Number(head.head_sequence) + 1, head.head_signature, new Date().toISOString()); this.signSegmentInventory(false); this.db.exec("COMMIT"); } catch (error) { this.db.exec("ROLLBACK"); throw error; } }
