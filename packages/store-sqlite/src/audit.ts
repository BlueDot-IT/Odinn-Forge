...
export class SqliteAuditStore {
  ...
  async verifyIntegrity({ allowUnsigned = true } = {}) {
    ...
    const segments = this.db.prepare("SELECT * FROM audit_segments ORDER BY id").all() as Row[]; const retainedSegments = archiveSegments(segments, retainedSequence); const retainedBoundaries = new Set(retainedSegments.map((segment) => segment.lastSequence)); const archived = retainedArchive ? verifyArchivedHistory(String(retainedArchive.path), retainedSequence, keyring, retainedBoundaries) : { valid: retainedSequence === 0, lastSignature: null, boundaries: new Map<number, string>() }; if (!archived.valid || (retainedSequence > 0 && archived.lastSignature !== retained.retained_signature)) failures.push({ runId: "", reason: "retained audit archive chain mismatch" }); if (retainedArchive && JSON.stringify(retainedArchive.manifest.segments) !== JSON.stringify(retainedSegments)) failures.push({ runId: "", reason: "retained audit segment inventory mismatch" }); let expectedFirst = 1; let priorFinal: string | null = null; let openSegments = 0;
