...
export class SqliteAuditStore {
  ...
  async verifyIntegrity({ allowUnsigned = true } = {}) {
    ...
    const segments = this.db.prepare("SELECT * FROM audit_segments ORDER BY id").all() as Row[]; const retainedBoundaries = new Set(segments.filter((segment) => segment.last_sequence !== null && Number(segment.last_sequence) <= retainedSequence).map((segment) => Number(segment.last_sequence))); const archived = retainedArchive ? verifyArchivedHistory(String(retainedArchive.path), retainedSequence, keyring, retainedBoundaries) : { valid: retainedSequence === 0, lastSignature: null, boundaries: new Map<number, string>() }; if (!archived.valid || (retainedSequence > 0 && archived.lastSignature !== retained.retained_signature)) failures.push({ runId: "", reason: "retained audit archive chain mismatch" }); let expectedFirst = 1; let priorFinal: string | null = null; let openSegments = 0;
  ...
  async exportArchive(path: string, throughSequence?: number) {
    const archivePath = resolve(path); const manifestPath = `${archivePath}.manifest.json`; this.db.exec("BEGIN IMMEDIATE");
    try {
...
function verifyArchivedHistory(path: string, throughSequence: number, keyring: Keyring, requestedBoundaries: Set<number>) {
  ...
  return { valid: valid && cursor === throughSequence && requestedBoundaries.size === boundaries.size, lastSignature: previous, boundaries };
}
