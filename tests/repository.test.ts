...
test("draft GitHub releases hand npm publication to the protected workflow", async () => {
  ...
  assert.match(verifyJob, /ODINN_SLO_EXPECTED_SHA: \$\{\{ env\.ODINN_RELEASE_COMMIT \}\}/u);
  assert.match(
    release,
  ...
  assert.match(stageJob, /bash scripts\/release\/download-draft-assets\.sh "\$GITHUB_REPOSITORY" "\$RELEASE_ID" downloaded-release-assets/u);
  assert.doesNotMatch(stageJob, /gh release download "\$TAG" --dir downloaded-release-assets/u);
  assert.match(stageJob, /verify-attested-assets\.ts release-assets downloaded-release-assets \\\s+"\$GITHUB_REPOSITORY" "\$EXPECTED_COMMIT" "\$ATTESTATION_RUN_ID"/u);
