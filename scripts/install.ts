...
import { HOSTILE_NODE_ENVIRONMENT_VARIABLES, HOSTILE_WINDOWS_RUNTIME_ENVIRONMENT_VARIABLES } from "./release/standalone-launchers.ts";
...
function installedWindowsLauncher(
  ...
  const activationPrelude = activation ? installedWindowsActivationPrelude(activation, physicalPathAssertion) : "";
...
function installedWindowsActivationPrelude(
  ...
  return `if exist "${marker}" (\r\n  set "ODINN_ACTIVATION_ROOT=${activation.targetRoot}"\r\n  set "ODINN_ACTIVATION_RUNTIME_DIR=${runtimeDirectory}"\r\n  set "ODINN_ACTIVATION_NODE=${runtime}"\r\n  set "ODINN_ACTIVATION_RUNTIME_SHA256=${activation.runtimeSha256}"\r\n  "%ODINN_POWERSHELL%" -NoLogo -NoProfile -NonInteractive -ExecutionPolicy Bypass -Command "${physicalPathAssertion};if($env:ODINN_ACTIVATION_RUNTIME_SHA256 -cnotmatch '^[a-f0-9]{64}$'){exit 126};Assert-OdinnPhysicalPath $env:ODINN_ACTIVATION_ROOT;Assert-OdinnPhysicalPath $env:ODINN_ACTIVATION_RUNTIME_DIR;Assert-OdinnPhysicalPath $env:ODINN_ACTIVATION_NODE;$i=Get-Item -LiteralPath $env:ODINN_ACTIVATION_NODE -Force -ErrorAction Stop;if($i.PSIsContainer){exit 126};if((Get-FileHash -LiteralPath $env:ODINN_ACTIVATION_NODE -Algorithm SHA256).Hash.ToLowerInvariant() -ne $env:ODINN_ACTIVATION_RUNTIME_SHA256){exit 126}"\r\n  if errorlevel 1 (echo Odinn activation runtime identity check failed 1>&2 & exit /b 126)\r\n  "%ODINN_ACTIVATION_NODE%" "${installer}" ${common} >nul\r\n  if errorlevel 1 (echo Odinn launcher activation reconciliation failed 1>&2 & exit /b 126)\r\n)\r\n`;
}
