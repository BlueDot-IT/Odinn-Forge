...
const ODINN_SUPPORTED_CASES = 4;
const ODINN_UNSUPPORTED_CASES = 3;
const ODINN_EXECUTED_TRIALS = 20;
const ODINN_UNSUPPORTED_TRIALS = 15;
...
function validate(reports: Report[], expectedCommit: string): void {
  ...
  for (const report of reports) {
    ...
    if (adapter === "odinn-forge") {
      assert(
        rows.filter((item) => item.trialsExecuted === 5 && item.trialsUnsupported === 0).length === ODINN_SUPPORTED_CASES,
        "Odinn one-shot adapter must execute exactly four supported cases"
      );
      assert(
        rows.filter((item) => item.trialsExecuted === 0 && item.trialsUnsupported === 5).length === ODINN_UNSUPPORTED_CASES,
        "Odinn one-shot adapter must mark exactly three process-dependent cases unsupported"
      );
      assert(report.summary[0].trialsExecuted === ODINN_EXECUTED_TRIALS, "Odinn one-shot adapter must execute exactly 20 trials");
      assert(report.summary[0].trialsUnsupported === ODINN_UNSUPPORTED_TRIALS, "Odinn one-shot adapter must mark exactly 15 trials unsupported");
      assert(report.summary[0].trialsFailed === 0, "Odinn supported benchmark trials must not fail");
      assert(report.summary[0].verified === ODINN_EXECUTED_TRIALS, "Odinn supported benchmark trials must all verify");
    }
  }
