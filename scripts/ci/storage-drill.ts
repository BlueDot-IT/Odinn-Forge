...
import { FileAuditStore, FileRecordStore } from "../../packages/store-file/src/index.ts";
...
try {
  ...
  const restoredLedger = createRunLedger({ stateDir: restored, workspaceRoot: root });
  const restoredJobs = new SqliteJobStore(restoredLedger);
