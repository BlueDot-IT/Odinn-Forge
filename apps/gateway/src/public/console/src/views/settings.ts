...
export function renderOptions(values: unknown[], current: unknown): string { return values.map((value) => '<option value="' + escapeHtml(value) + '"' + selectedOption(value, current) + ">" + escapeHtml(value) + "</option>").join(""); }
